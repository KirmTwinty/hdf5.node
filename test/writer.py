...
class SwmrReader(Process):
    def __init__(self, event, fname, grpname, dsetname, timeout = 2.0):
    ...
    def run(self):
        ...
        assert f.swmr_mode
        dset = f[self._grpname][self._dsetname]
        try:
...
class SwmrWriter(Process):
    def __init__(self, event, fname, grpname, dsetname):
    ...
    def run(self):
        ...
        try:
            grp = f.create_group(self._grpname)
            arr = np.array([1,2,3,4])
            dset = grp.create_dataset(self._dsetname, chunks=(2,), maxshape=(None,), data=arr)
            ...
            for i in range(6):
                ...
                self._event.set()
                self.log.info("Now sleeping for 1 second.")
                time.sleep(1)
        finally:
            f.close()
...
if __name__ == "__main__":
    if h5py.version.hdf5_version > '1.10':
        ...
        event = Event()
        reader = SwmrReader(event, fname, grpname, dsetname)
        writer = SwmrWriter(event, fname, grpname, dsetname)
        ...
        print('Please update your HDF5 library (version not compatible with SWMR).')
